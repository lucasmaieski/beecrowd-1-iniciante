...
def imprimir(lista, par_ou_impar):
  for i, valor in enumerate(lista):
    print(f"{par_ou_impar}[{i}] = {valor}")

def limpa_lista(lista, par_ou_impar):
  if len(lista) == 5:
    imprimir(lista, par_ou_impar)
    return []
  return lista
...
def principal():
  ...
  for _ in range(15):
    ...
    if n % 2 == 0:
      pares.append(n)
      pares = limpa_lista(pares, "par")
    else:
      impares.append(n)
      impares = limpa_lista(impares, "impar")
